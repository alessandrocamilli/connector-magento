...
##############################################################################
#
#    Author: Alessandro Camilli, Andrea Colangelo, Walter Antolini
#    Copyright 2015 Openforce di Camilli Alessandro
#
...
class ProductTemplateBatchImporter(DelayedBatchImporter):
    """ Import the Magento Configurable Products .
    ...
    def run(self, filters=None):
        ...
        record_ids = self.backend_adapter.search(filters,
                                                 from_date=from_date,
                                                 to_date=to_date)
        _logger.info('search for magento configurable products %s returned %s',
                     filters, record_ids)
        for record_id in record_ids:
            self._import_record(record_id)

ProductTemplateBatchImport = ProductTemplateBatchImporter  # deprecated


@magento
class TemplateCatalogImageImporter(Importer):
...
class ProductTemplateImportMapper(ImportMapper):
    ...
    direct = [('name', 'name'),
              ...
              ('weight', 'weight'),
              # ('cost', 'standard_price'), only product.product
              ('short_description', 'description_sale'),
              # ('sku', 'default_code'), only product.product
    ...
    def is_active(self, record):
        mapper = self.unit_for(IsActiveProductTemplateImportMapper)
        return mapper.map_record(record).values(**self.options)

    @mapping
    def price(self, record):
        mapper = self.unit_for(PriceProductTemplateImportMapper)
        return mapper.map_record(record).values(**self.options)

    @mapping
    def type(self, record):
        if record['type_id'] in ('simple', 'configurable'):
            return {'type': 'product'}
        return
...
@magento
class ProductTemplateImporter(MagentoImporter):
    _model_name = ['magento.product.template']

    _base_mapper = ProductTemplateImportMapper
    ...
    def _must_skip(self):
        ...
        """
        if self.magento_record['type_id'] == 'simple':
    ...
    def _after_import(self, binding):
        ...
        translation_importer.run(self.magento_id, binding.id,
                                 mapper_class=ProductTemplateImportMapper)
        image_importer = self.unit_for(TemplateCatalogImageImporter)
...
ProductTemplateImport = ProductTemplateImporter  # deprecated


@magento
class PriceProductTemplateImportMapper(ImportMapper):
    _model_name = 'magento.product.template'
...
@magento
class IsActiveProductTemplateImportMapper(ImportMapper):
    _model_name = 'magento.product.template'
